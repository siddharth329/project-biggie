...
public class JobExecutor {
    ...
    public void processJobs(ConversionQueue job) {
        ...
        try {
            ...
            workingMemory = mediaConversionPipeline.execute(new PipelineWorkingMemory(job));
            log.info("Job {} completed gracefully", jobId);

            if (workingMemory.getStatus()) jobQueueService.completeJob(jobId);
            else throw new RuntimeException(workingMemory.getErrorMessage());
        } catch (Exception e) {
